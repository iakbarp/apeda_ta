...
@section('title', 'Aplikasi Pembangunan Daerah | Tambah Pengguna Desa')
@section('nav')
...
@endsection
@section('content')
    @include('user.desa..pengurus.form')
    {{--optionjob--}}
    <script>
        var $joboption1 = [];
        var $joboption2 = [];
                                            ...
                                            <div id="isiinputuser">
                                                @for ($i=0;$i<1;$i++)
                                                    <div class="row form-group has-feedback" data-no="{{$i}}">
                                                        ...
                                                        <div class="col-md-4">
                                                            <select placeholder="Pilih Desa" id="village_id[]"
                                                                    class="form-control wilayah" data-id="{{$i}}"
                                                                    name="village_id[]" required autofocus>
                                                                <option value="" selected disabled> Pilih Desa</option>
                                                                @foreach(\App\village::orderBy('name','asc')->where('district_id', '=', Auth::user()->district_id)->get() as $value)
                                                        ...
                                                        <div class="col-md-2">
                                                            @foreach(\App\trDataJobDesc::find($id=['3']) as $row)
                                                                <input id="job_id[]" type="hidden"
                                                                       class="form-control seksi" name="job_id[]"
                                                        ...
                                                        <div class="col-md-2">
                                                            ...
                                                            @foreach(\App\trDataPosisition::find($id=['3']) as $row)
                                                                <input id="posisition_id[]" type="hidden"
                                                                       ...
                                                                       name="posisition_id[]" value="{{$row->id}}"
                                                                       readonly>
                                                            @endforeach

                                                        </div>
                                                        <div class="col-md-2">
                                                            @foreach(\App\status::find($id=['4']) as $row)
                                                                <input id="role_id[]" type="hidden" class="form-control"
                                                                       name="role_id[]" value="{{$row->id}}" readonly>
                                                            @endforeach
                                                        </div>
                                                        <div class="col-md-2">
                                                        ...
                                                        <div class="col-md-2">
                                                            <input id="district_id[]" type="hidden" class="form-control"
                                                                   name="district_id[]"
...
//            console.log($isiinputuser.length);
            $(document).on('change', '#jumlah', function () {
                ...
                for ($i = 0; $i < $ulang; $i++) {
                    ...
                    $tanggal.push('<div class="col-md-4"><select placeholder="Pilih tanggal" id="tgl_lahir[]"\n' +
                        ...
                        '                                                                            required autofocus>' +
                        $isiinputuser.find('.wilayah').clone().attr('class', 'form-control' + $i).get(0).innerHTML + '</select></div>');
                    $desa.push('<div class="col-md-4"><select placeholder="Pilih Desa" id="village_id[]"\n' +
                        '                                                                            class="form-control wilayah" data-id="' +
                        $i + '"\n' +
                        '                                                                            name="village_id[]"\n' +
                        '                                                                            required autofocus>' +
                ...
                $isi = '';
                for ($i = 0; $i < $ulang; $i++) {
                    $isi += '<div class="row form-group has-feedback" data-no="' + $i + '">' + $nik[$i] + $name + $email[$i] + $city[$i] + $district[$i] + $desa + $tempat[$i] + $tanggal[$i] + $seksi[$i] + $jabatan[$i] + $hak + '</div>';
                }
    ...
    {{--keyup--}}
    @include('user.desa.pengurus.tabUser.table')
@endsection