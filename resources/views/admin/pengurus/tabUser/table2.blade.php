...
<script>
    $('#pagiuser').on('click', '.page', function (asd) {
        ...
        getData3($index2, $pagi);
    });
</script>
{{--pagination--}}
{{--getdata pagination--}}
<script>
    function getData3($index2, $pagi) {
        $('.content .row').css("opacity", 0.4);
        $('#load2').fadeIn(500);
        $('#job' + $index2 + ' table tbody').empty().append('<tr><td colspan="6">loading...</td></tr>');
        $.ajax({
...
<script>
    ...
    function searchsurat2() {
        ...
        getData3($index2, pagination2[prefix2 + $index2]);
...
<script>
    ...
    $('#data3 form').on('submit', function (e) {
        if (navigator.onLine) {
            $('#data3 form :input').prop('readonly', true);
            $('#data3 form .email').prop('readonly', true);
            $('#data3 .seksi').addClass('disabled');
            $('#data3 .seksi option:not(:selected)').attr('disabled', 'disabled');
            $('#data3 .opsi').addClass('disabled');
            $('#data3 .opsi option:not(:selected)').attr('disabled', 'disabled');
            $(':input[type="submit"]').prop('disabled', true);
            $('#data3 form #berubah').hide();
            $('#data3 form #loading1').show();
            $('#data3 form #progresssurat').show();
            $x = 0;
            $('#data3 form #progresssurat').empty().append($x + '/' + countSend + ' Proses Kirim (Jangan Tutup/Refresh Halaman)');
    ...
    function send($x, entit) {
        $.ajax({
            ...
            // async: false,
            type: "post",
            data: new FormData($('#data3 form')[0]),
            contentType: false,
            processData: false,
            success: function (data) {
                console.log(data);
    ...
    function doneSend() {
        ...
        // $('#data2 form')[0].reset();
        $('#data3 form #entitySend').val(countSend);
        console.log(data);
        $('#data3 form .email').prop('readonly', false);
        $('#data3 form #berubah').show();
        $('#data3 form #loading1').hide();
        $('#data3 form #progresssurat').hide();
        $('#data3 form :input').prop('readonly', false);
        $('#data3 .seksi option:not(:selected)').attr('disabled', false);
        $('#data3 .seksi').removeClass('disabled');
        $('#data3 .opsi option:not(:selected)').attr('disabled', false);
        $('#data3 .opsi').removeClass('disabled');
...
{{--cek email nip--}}
<script>
    $('#data3').on('input', '.nik', function (asd) {
        var NIK = [];
        $statusnik = 0;
        $call = $('#data3.nik');
        label = $(this).val();
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data == 0 || $statusnik == 1) {
                    ...
                    $('#data3 .opsi').addClass('disabled');
                    $('#data3 .opsi option:not(:selected)').attr('disabled', 'disabled');
                    $('#data3 :input[type="submit"]').prop('disabled', true);
                    swal({
                        title: 'notice',
                    ...
                    $('#data3 form #berubah').empty().append('NIK telah dipakai!');
    ...
    $('#data3').on('input', '.email', function (asd) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data == 0 || $statusemail == 1) {
                    ...
                    $('#data3 form #berubah').empty().append('email telah dipakai!');
                    // $( '#jadi-data form input:not(.'+urutancek+')' ).prop('readonly', true);
                    // $( '#jadi-data form .text1' ).prop('readonly', true);
                    // $('#bantu').empty().append(isicek1 + isicek2);
                    // $(':input[type="submit"]').prop('disabled', true);
    ...
    function noticecek() {
        ...
        $('#data3 .opsi').removeClass('disabled');
        $('#data3 .opsi option:not(:selected)').attr('disabled', false);
        $('#data3 :input[type="submit"]').prop('disabled', false);
        $('#data3 form #berubah').empty().append('Submit');
    }
</script>
{{--cek email--}}
