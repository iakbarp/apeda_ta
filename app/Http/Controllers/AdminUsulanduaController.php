...
class AdminUsulanduaController extends Controller
{
    public function index()
    {
        $base_url = url('/admin/request');
        $job = trDataCategory::where('job_id', 3)->get();
        $data2 = mst_data::where('job_id', 3)->orderBy('created_at', 'desc')->get();
        $job2 = array();
        $job3 = array();
        foreach ($job as $row) {
            $job2[] = array($row->id);
...
//        dd($job3);
        $a = -1;
        return view('admin.request.indexdua', compact('base_url', 'job_id', 'job', 'data2', 'job2', 'a', 'job3'));
    }
...
//                    $filesize = $row->getClientSize();
    ...
    {
        $products = mst_data::where('job_id', 3)->orderBy('id', 'desc')->get();
        Carbon::setLocale('id');
    ...
    {
        $products = mst_data::onlyTrashed()->where('job_id', 3)->orderBy('id', 'desc')->get();
        Carbon::setLocale('id');
        setlocale(LC_TIME, 'Indonesian');
        return DataTables::of($products)
            ->addColumn('user', function ($products) {
