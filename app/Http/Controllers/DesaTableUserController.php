...
class DesaTableUserController extends Controller
{
    public function index($users)
    {
        ...
        if ($users == 'user' || $users == 'letter') {
            return view('user.desa.pengurus.user', compact('req', 'jobCategory', 'user', 'users', 'category'));
        } else {
    ...
    public function apiUser(Request $request)
    {
        ...
        foreach ($datas as $data) {
            ...
            $data1[] = array('name' => $data->name, 'id' => $data->id, 'nik' => $data->nik, 'posisition' => $posisition, 'img' => asset($ava));
        }
...
//                $object['due'] = Carbon::createFromFormat('Y-m-d H:i:s', $key->created_at)->formatLocalized('%A, %d %B %Y');
    ...
    public function storesuratplus(Request $r)
    {
        ...
        Mail::send('user.desa.pengurus.mail.send', $data, function ($message) use ($data) {
...
//            'remember_token' => Str::random(12),
    ...
    {
        return view('user.desa.pengurus.mail.send');
    }

    public function ceknik(Request $r)
    {
        if ($r->mode == 1) {
            $cek = User::all()->pluck('nik');
            foreach ($cek as $a) {
                if ($r->cek == $a) {
                    $status = 0;
                    break;
